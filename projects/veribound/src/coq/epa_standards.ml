...
module EPA_Environmental_Standards =
 struct
  (** val aqi_good_limit : RbaseSymbolsImpl.coq_R **)

  let aqi_good_limit =
    q2R { qnum = (Zpos (XO (XO (XO (XI (XI (XI XH))))))); qden = (XO (XI (XO
      XH))) }

  (** val epa_measurement_precision : RbaseSymbolsImpl.coq_R **)

  let epa_measurement_precision =
    rdiv (q2R { qnum = (Zpos (XO (XI (XO XH)))); qden = (XO (XI (XO XH))) })
      (q2R { qnum = (Zpos (XO (XO (XO (XO (XI (XO (XO (XO (XI (XI (XI (XO (XO
        XH)))))))))))))); qden = (XO (XI (XO XH))) })

  (** val nox_emission_limit : RbaseSymbolsImpl.coq_R **)

  let nox_emission_limit =
    q2R { qnum = (Zpos (XO (XO (XI (XO XH))))); qden = (XO (XO (XI (XO (XO
      (XI XH)))))) }
 end
