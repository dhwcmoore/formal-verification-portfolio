...
(** val b2R : z -> z -> binary_float -> RbaseSymbolsImpl.coq_R **)
...
type verifiedBoundary = { lower_float : float64; upper_float : float64;
                          category : string }

type verifiedDomain = { domain_name : string;
...
let in_boundary_range x b =
  ...
  (match rle_dec lo xr with
   | Left -> (match rle_dec xr hi with
              | Left -> True
              | Right -> False)
   | Right -> False)
...
let rec find_category_flocq bs x =
  match bs with
  | Nil -> None
  | Cons (b, bs') ->
    (match in_boundary_range x b with
     | True -> Some b.category
     | False -> find_category_flocq bs' x)
